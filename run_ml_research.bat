...
set "REPORT_FILE=%REPORT_DIR%\ml_research.txt"

echo TradingBot one-click ML research
...
echo.
echo Running locked ML Decision Engine v1 research...
(
    echo TradingBot one-click ML research report
    ...
    echo Slippage: 0.1%%
    echo.
    "%PYTHON%" -m trading_bot.app ml-research --symbols "config\swedish_large_caps.txt" --output-dir "data"
) > "%REPORT_FILE%" 2>&1
if errorlevel 1 goto research_error

echo.
echo ML RESEARCH COMPLETE
